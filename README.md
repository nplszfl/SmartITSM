...
| ⏰ **SLA预警服务** | SLA倒计时监控、超时预警、达标率统计 | 合规率提升95% |
| 📦 **资产自动盘点** | 盘点任务生成、使用率统计、变更追踪 | 盘点效率提升10倍 |
...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 智能服务层                                                     │   │
│  │ (工单分类/根因分析/预测维护/SLA预警/资产盘点/自动处置)           │   │
│  └─────────────────────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────────────────────┤
│                    DeepSeek API                                      │
└─────────────────────────────────────────────────────────────────────┘
...
```
GET /api/v1/sla/countdown/{ticketId}

响应：
{
  "ticketId": "TKT-12345",
  "tier": "P1",
  "targetResponseTime": 3600,
  "targetResolutionTime": 14400,
  "elapsedSeconds": 2700,
  "responseRemaining": 900,
  "resolutionRemaining": 11700,
...
{
  "totalTickets": 1250,
  "responseCompliant": 1180,
  "resolutionCompliant": 1150,
  "responseRate": 94.4,
  "resolutionRate": 92.0,
  "tierStats": [...]
}
...
```
POST /api/v1/inventory/generate-task

请求体：
{
  "scope": "all",
  "includeAssets": ["SRV-001", "SRV-002"],
  "dueDate": "2026-06-15"
}
...
### 5. 智能工单分类服务 (smart-ticket-classification-service)

独立微服务，负责AI工单分类、优先级判定、服务台分配：
- **自动分类**：硬件、软件、网络、安全、账户等类别
- **优先级判定**：紧急、高、中、低自动评估
- **服务台分配**：基于分类和负载自动路由到最佳处理团队
- **SLA预估**：智能预测解决时间

### 6. SLA预警服务 (sla-alert-service)

SLA倒计时监控、超时预警、达标率统计：
- **实时监控**：工单SLA剩余时间倒计时
- **超时预警**：多级别预警（提醒→警告→危急）
- **达标率统计**：按团队、分类、时间段统计SLA达成率
- **历史分析**：SLA合规趋势分析

### 7. 资产自动盘点服务 (asset-inventory-service)
